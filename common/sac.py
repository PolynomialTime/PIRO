...
class SAC:
    ...
    def __init__(self, env_fn, replay_buffer, k=1, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
            ...
            save_freq=10, device=torch.device("cpu"), automatic_alpha_tuning=True, reinitialize=True,sa=False,
            **kwargs):
        """
    ...
    def test_agent(self):
        ...
        for j in range(self.num_test_episodes):
            ...
            if self.sa:
                obs = np.zeros((self.max_ep_len, o.shape[0]+self.get_action(o, True).shape[0]))
            for t in range(self.max_ep_len):
                # Take deterministic actions at test time?
                o, _, _, _ = self.test_env.step(self.get_action(o, True))
                if self.sa:
                    obs[t]=np.concatenate([o.copy(),self.get_action(o, True)])
                else:
                    obs[t] = o.copy()
        ...
        for t in range(100):
            ...
            while not d:
                ...
                if d or ep_len==self.max_ep_len:
                    print('-----sumreward is -------',sumreward)
                    if sumreward>5300:
                        ep_lens.append(ep_len)
                        len_l.append(ep_lens)
                    ...
                    o, ep_len = self.env.reset(), 0
                    break
        np.save('humanoid_actions.npy',actions_l)
        np.save('humanoid_next_states.npy',next_states_l)
        np.save('humanoid_states.npy',states_l)
        np.save('humanoid_reward.npy',rewards_l)
        np.save('humanoid_dones.npy',dones_l)
        np.save('humanoid_lens.npy',len_l)
