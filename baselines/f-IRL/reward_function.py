"""Constructs deep network reward models and Utilities for processing reward networks."""
...
class RwdFromRwdNetFIRL(RewardFn):
    ...
    def __call__(
        ...
        # Fix: Use a safer way to determine device without relying on device attribute
        # First try to get device from parameters, fallback to CPU if no parameters or device attribute
        device = th.device("cpu")  # Default to CPU
        try:
            # Try to get device from parameters
            for param in self.rwd_net.parameters():
                device = param.device
                break
        except:
            pass  # If no parameters or error, use default CPU
            
        # Use clone().detach() or specify dtype explicitly to avoid type mismatch
        return self.rwd_net(th.tensor(state, dtype=th.float32, device=device)).cpu().detach().numpy().flatten()
